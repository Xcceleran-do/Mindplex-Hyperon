...
=======
## Mindplex-Hyperon Copilot instructions

### Big picture architecture
- Core work lives under experiments: MeTTa pattern mining modules plus a UI for visualization and chat. 
- The interactive app is in [experiments/atomspace_visualizer/src](../experiments/atomspace_visualizer/src), with the main columnar view in [experiments/atomspace_visualizer/src/AppColumnar.tsx](../experiments/atomspace_visualizer/src/AppColumnar.tsx).
- UI components of interest: chat in [experiments/atomspace_visualizer/src/components/ChatInterface/ChatInterface.tsx](../experiments/atomspace_visualizer/src/components/ChatInterface/ChatInterface.tsx), mining controls in [experiments/atomspace_visualizer/src/components/MiningPanel/MiningPanel.tsx](../experiments/atomspace_visualizer/src/components/MiningPanel/MiningPanel.tsx), columnar rendering in [experiments/atomspace_visualizer/src/components/ColumnarVisualizer/ColumnarVisualizer.tsx](../experiments/atomspace_visualizer/src/components/ColumnarVisualizer/ColumnarVisualizer.tsx).
- MeTTa mining logic is in [experiments/frequent-pattern-miner](../experiments/frequent-pattern-miner) and wrapped by [experiments/pattern-miner/pattern-miner.metta](../experiments/pattern-miner/pattern-miner.metta); patterns are returned as supportOf entries.

### Data flow and integration points
- Mining flow: UI triggers backend mining; patterns returned with support counts are visualized and can be analyzed by chat. See endpoints in [experiments/README.md](../experiments/README.md).
- Chat auto-opens when conjunct size changes; this is coordinated by `ChatInterface` reacting to `conjunctSize` updates in the UI.
- Backend endpoints (ports and paths) are documented in [experiments/README.md](../experiments/README.md); keep UI requests aligned with those routes.

### Dev workflows
- Full stack (frontend + backend): run [experiments/start_all.sh](../experiments/start_all.sh) and stop with [experiments/stop_all.sh](../experiments/stop_all.sh).
- Frontend-only dev: run Vite in [experiments/atomspace_visualizer](../experiments/atomspace_visualizer) (see [experiments/atomspace_visualizer/README.md](../experiments/atomspace_visualizer/README.md)).

### Project conventions (must follow)
- MeTTa formatting, comment style, and naming rules are defined in [CONVENTION.md](../CONVENTION.md) (use `;;` comments, camelCase names, and the formatting rules for multi-line forms).
- Tests: place new MeTTa tests in a tests subfolder and use the -test suffix (examples in [experiments/frequent-pattern-miner/tests](../experiments/frequent-pattern-miner/tests) and [experiments/pattern-miner/tests](../experiments/pattern-miner/tests)).
- Contribution rules and branch naming are in [CONTRIBUTING.md](../CONTRIBUTING.md); ensure changes and PRs align with those guidelines.
>>>>>>> Stashed changes
