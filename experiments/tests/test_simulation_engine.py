...
class TestSimulationEngine(unittest.TestCase):
    def setUp(self) -> None:
        self._saved_dataset_facts = list(mining_api.chainer_dataset_facts)
        self._saved_rule_atoms = list(mining_api.chainer_rule_atoms)
        mining_api.chainer_dataset_facts = []
        mining_api.chainer_rule_atoms = []

    def tearDown(self) -> None:
        mining_api.chainer_dataset_facts = self._saved_dataset_facts
        mining_api.chainer_rule_atoms = self._saved_rule_atoms
    ...
    def test_build_simulation_fact_atoms_uses_active_legacy_length_predicate(self) -> None:
        ...
        facts = mining_api.build_simulation_fact_atoms(
    ...
    def test_simulate_engagement_aggregates_bucket_scores(self) -> None:
        ...
        with patch.object(mining_api, "reload_petta_dataset_if_ready", return_value={"status": "unchanged"}), \
             patch.object(mining_api, "run_simulation_worker", return_value=worker_result):
            result = mining_api.simulate_engagement(
    ...
    def test_build_simulation_explanation_maps_proof_to_rule_and_fact(self) -> None:
        explanation = mining_api.build_simulation_explanation(
            article_id="H_demo",
            hypothetical_facts=[
