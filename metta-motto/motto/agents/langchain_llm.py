...
class LangChainLLmAgent(Agent):
    ...
    def _collect_tools_and_structure(self, functions):
        ...
        for func in functions:
            # Default to Tool type if doc_type is not present (for MeTTa function definitions)
            doc_type = func.get('doc_type', DocType.Tool.value)
            
            if doc_type == DocType.Tool.value:
                tool = ai_importer.imported_modules["langchain_core.tools"].StructuredTool.from_function(
                    name=func["name"],
                    description=func["description"],
                    args_schema=pydantic_model_from_json_schema("funcInput", func.get("parameters", {})),
                    func=mock_func
                )
                tools.append(tool)
            if doc_type == DocType.OutputStructure.value:
    ...
    def __call__(self, messages, functions=[]):
        try:
            ...
            if len(functions) > 0:
                ...
                if len(tools) > 0:
                    # Force tool usage by configuring the model to require tool calls
                    self.model_to_invoke = self.model_to_invoke.bind_tools(tools, tool_choice="any")
